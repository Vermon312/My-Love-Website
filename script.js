...
/* ============================
   ...
   Starts below hero
============================ */
(function pageSkyModule() {
  const sky = document.getElementById('page-sky');
  const hero = document.querySelector('.hero');
  if (!sky || !hero) return;

  // 1) Make the sky start exactly below the hero
  function positionSky() {
    // Use the full rendered height of the hero (includes responsive height)
    const h = hero.getBoundingClientRect().height;
    // Set the 'top' so the sky begins below the hero
    sky.style.top = `${h}px`;
  }
  positionSky();
  window.addEventListener('resize', () => {
    // Debounce
    clearTimeout(positionSky._t);
    positionSky._t = setTimeout(positionSky, 120);
  });

  // 2) Stars
  function addPageStars() {
    Array.from(sky.querySelectorAll('.p-star')).forEach(n => n.remove());
    const rect = sky.getBoundingClientRect();
    // density scales with area, capped to stay performant
    const count = Math.min(140, Math.max(80, Math.floor((rect.width * rect.height) / 12000)));
    for (let i = 0; i < count; i++) {
      ...
      const twi  = (2.6 + Math.random() * 3.6).toFixed(1) + 's';
      s.style.setProperty('--size', size + 'px');
      s.style.setProperty('--twinkle', twi);
      s.style.left = left + '%';
      s.style.top  = top + '%';
  ...
  if (!svg) {
    ...
    svg.setAttribute('width', '100%');
    svg.setAttribute('height', '100%');
    svg.setAttribute('viewBox', '0 0 1000 2000'); // taller canvas to cover long pages
    sky.appendChild(svg);
  }
  ...
  function drawConstellation(nodes, edges, options) {
    ...
    edges.forEach(([a, b]) => {
      ...
      line.setAttribute('class', 'p-const-line');
      g.appendChild(line);
    });
    ...
    nodes.forEach(([x, y]) => {
      ...
      c.setAttribute('r', 2.9);
      c.setAttribute('class', `p-const-node ${options.nodeClass || ''}`);
      g.appendChild(c);
    });
    ...
    if (options.label) {
      ...
      text.setAttribute('class', 'p-const-label');
      text.textContent = options.label;
      g.appendChild(text);
    }
  }

  // 4) Place Cancer & Sagittarius a few times down the page
  //    We repeat them at different Y bands so the sky feels present as you scroll.
  function drawAllConstellations() {
    // Clear prior drawings
    Array.from(svg.querySelectorAll('g')).forEach(n => n.remove());

    // Base shapes (same as hero, but we will shift them vertically in copies)
    const cancerNodesBase = [
      [210, 260], [250, 220], [300, 250], [340, 210],
      [280, 300], [240, 310]
    ];
    ...
    const sagEdges = [[0,1],[1,2],[2,3],[3,4],[1,5],[5,6],[6,7]];

    // Copies at different vertical offsets (in SVG user space)
    const yBands = [100, 700, 1300, 1800]; // adjust/add for longer pages
    yBands.forEach((yoff, i) => {
      const cancerNodes = cancerNodesBase.map(([x, y]) => [x, y + yoff]);
      const sagNodes    = sagNodesBase.map(([x, y])    => [x, y + yoff]);

      drawConstellation(cancerNodes, cancerEdges, {
        nodeClass: 'cancer',
        label: i === 0 ? 'Cancer ♋' : '',  // label first copy only (cleaner)
        labelPos: [180, yoff - 50]
      });

      drawConstellation(sagNodes, sagEdges, {
        nodeClass: 'sagittarius',
        label: i === 0 ? 'Sagittarius ♐' : '',
        labelPos: [680, yoff - 80]
      });
    });
  }
  ...
  // Refresh stars on resize (debounced)
  let t;
  window.addEventListener('resize', () => {
    clearTimeout(t);
    t = setTimeout(() => {
